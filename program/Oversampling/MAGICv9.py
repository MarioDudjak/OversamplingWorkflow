            ...
            for distance, idx in zip(distances[0][1:], indices[0][1:]):
                if int(y[idx]) != minorityLabel:
                    if neighbourhood_size == 0:
                        rnd_idx = np.random.choice(indices[0][1:])
                        while int(y[rnd_idx]) != minorityLabel:
                            rnd_idx = np.random.choice(indices[0][1:])
                        closes_minority_nn = X[rnd_idx]
                        minority_distance = dst.euclidean(closes_minority_nn, instance)

                        scale = (minority_distance - distance) / minority_distance
                        try:
                            alfa = np.random.uniform(low=scale, high=1)
                        except:
                            alfa = 1
                        beta = 1 - alfa
                        s = alfa * instance + beta * closes_minority_nn
                        radius = dst.euclidean(instance, s)
