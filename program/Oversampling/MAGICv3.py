...
class MAGICv3(Oversampler):
    ...
    def make_samples(self, X, y, N=100, k=5):
        ...
        maj_nbrs = NearestNeighbors(n_neighbors=len(majorityInstances), metric='euclidean').fit(majorityInstances)
        min_nbrs = NearestNeighbors(n_neighbors=len(minorityInstances), metric='euclidean').fit(minorityInstances)
        S = []  # Synthetic data set

        # Synthetic instances creation
        for instance in minorityInstances:
            min_distances, min_indices = min_nbrs.kneighbors(instance.reshape(1, -1), return_distance=True)
            maj_distances, maj_indices = maj_nbrs.kneighbors(instance.reshape(1, -1), return_distance=True)
            hmean_min_distance = hmean(min_distances[0][1:])
            hmean_maj_distance = hmean(maj_distances[0])

            neighbourhood = []
            for distance, idx in zip(min_distances[0][1:], min_indices[0][1:]):
                if distance < hmean_maj_distance:
