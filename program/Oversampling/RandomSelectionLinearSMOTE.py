...
class RandomSelectionLinearSMOTE(Oversampler):
    """RadnomSelectionLinearSMOTE Algorithm

        Creates synthetic instances of minority dataset in binary classification problems.

        Parameters
        ----------
        M : array_like(float), shape (n_minority_samples, n_features)
            Minority dataset.
    ...
    def make_samples(self, X, y, N, k):
        ...
        for i in range(count):
            ...
            s = random_minority_instance + np.random.uniform(0, 1) * (Xr - random_minority_instance)
            S.append(s)
